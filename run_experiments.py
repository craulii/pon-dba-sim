...
(XG-PON, IPACT vs GIANT vs QoSDBA, 8 ONUs).

Por cada escenario corre `repetitions` repeticiones con seeds distintos.
Guarda resultados consolidados en results/results.csv y, para los
...
def run_all():
    ...
    # Mantener el orden original de configs/scenarios.json
    ...
    if cycle_rows:
        out_path = os.path.join(RESULTS_DIR, "cycle_times.csv")
        with open(out_path, "w", newline="") as f:
