...
de control GTC de 125 $\mu$s de G.984.3 \cite{itu984-3} pero duplica la
tasa ascendente y cuadruplica la descendente \cite{itu987-1,itu987-2,itu987-3}. La Tabla
...
intacta y construir Fase 3 de forma aditiva: el camino broadcast que usan
GIANT y QoSDBA (\texttt{olt.py}, y la lógica de \texttt{onu.py} que
procesa el BWmap) queda igual que en Fase 2, mientras que IPACT introduce
un camino de
...
GIANT implementa dos fases \cite{giant}: GPA (\emph{Guaranteed Phase
Allocation}, T-CONT1 fijo e incondicional, T-CONT2 con contador SImax) y
SPA (\emph{Surplus Phase Allocation}, T-CONT4 con contador SImin). Durante
...
bajo sobrecarga sostenida. Estas dos correcciones son contribuciones
propias del equipo, documentadas como tales.

Durante las pruebas de humo también se encontró un error de
implementación con un impacto mucho mayor: el contador de T-CONT4 se
reiniciaba a SImin tras cualquier grant mayor a cero, incluso si la ONU
seguía congestionada. Esto generaba un \emph{duty cycle} sincronizado en
el que solo 8 de cada 32 tramas repartían ancho de banda de T-CONT4 entre
las 8 ONUs. El arreglo fue condicionar el reinicio del contador a que la
cola quedara realmente drenada (\texttt{grant} $\geq$ \texttt{demand}).
En una corrida de prueba a 400 Mbps/ONU, esto llevó la utilización del
canal de 36.2\% a 99.3\%, y el throughput de T-CONT4 de 485 a
2014 Mbps.
